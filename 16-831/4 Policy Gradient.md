...
We need the dynamics $p(s_{t+1}|s_t, a_t)$. Options:
...
Note: The baseline b does not have to a constant, like the state-dependent expected return below, as long as it does not affect the mean of the gradient estimator (meaning the gradient estimator is still unbiased).
...
We might want to constraint the policy based on the KL divergence:
...
Compared with standard policy gradient, PPO employs **importance sampling**. This mathematical trick allows you to estimate the gradient of a new policy using data collected by an old policy. You can run multiple steps of gradient descent on the _same_ batch of data. However, PPO is still **on-policy**.
